...
// ---------- storage helpers ----------
// This app's data (Selection File, Fabric Requirement, styles, users — everything) all flows
// through storeGet(key)/storeSet(key, value), which now hits a single shared database via the
// /api/data Vercel Function (see api/data.js + schema.sql) instead of the browser's own
// localStorage — that's what actually makes the app multi-user: everyone's browser reads and
// writes the same rows instead of each having its own private copy. The `shared` parameter is
// kept for call-site compatibility with earlier versions of this file but no longer changes
// behavior — every piece of data in this app is meant to be visible to the whole team, so there
// is no longer a "private" storage mode.
//
// If the API is unreachable — most commonly because you're running `npm run dev` (plain Vite,
// which doesn't serve /api routes) instead of `vercel dev`, or the Postgres integration hasn't
// been added to the Vercel project yet — both functions fall back to localStorage so the app
// still works for local development, just without team-wide sync. dbHealthy tracks which mode
// is actually active so the UI can tell you honestly instead of just claiming "shared" always.
let dbHealthy = true;
function reportDbHealth(ok){
  if(ok===dbHealthy) return;
  dbHealthy = ok;
  if(typeof window!=='undefined') window.dispatchEvent(new CustomEvent('embee-db-health', {detail: ok}));
}
...
async function storeSet(key,value,shared){
  try{
    ...
    reportDbHealth(false);
    console.error('shared database save failed, falling back to this browser only:',e);
    try{ localStorage.setItem('embee_'+key, JSON.stringify(value)); return true; }
    catch(e2){ console.error('storage set failed',e2); return false; }
  }
}
...
export default function App(){
  ...
  const [dbConnected,setDbConnected] = useState(null); // null=checking, true=shared DB working, false=fell back to this browser only
  ...
  useEffect(()=>{
    (async ()=>{
      ...
      setLoaded(true);
      setDbConnected(dbHealthy);
    })();
  },[]);
  useEffect(()=>{
    const onHealth = e => setDbConnected(e.detail);
    window.addEventListener('embee-db-health', onHealth);
    return ()=>window.removeEventListener('embee-db-health', onHealth);
  },[]);
        ...
        <div className="content">
          ...
          {page!=='dashboard' && (
            <div className="autosave-note" style={{marginBottom:14}}>
              ...
              {dbConnected===false ? 'Falling back to this browser only — shared database unreachable, data will not sync to teammates until this is fixed' : 'Auto-saved · shared with your whole team'}
            </div>
          )}
