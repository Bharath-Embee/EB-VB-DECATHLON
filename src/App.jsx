...
const SELECTION_MAPPING_FIELDS = [
  {key:'styleNo', label:'Style Number', guess:['style no','style number','rs','reference','style']},
  {key:'r3Code', label:'Iman / Model Code', guess:['model code','iman','iman code','r3 code']},
  {key:'description', label:'Style / Model Description', guess:['model name','style description','description']},
  {key:'cc', label:'CC', guess:['cc']},
  {key:'brand', label:'Brand', guess:['brand']},
  {key:'buyer', label:'Buyer', guess:['buyer','cpm']},
  {key:'color', label:'Colour', guess:['colour','color']},
  {key:'factory', label:'Factory', guess:['factory']},
  {key:'country', label:'Country', guess:['country']},
  {key:'company', label:'Company', guess:['company','sharing']},
...
function guessSelectionColumn(field, headers){
  for(const g of field.guess||[]){
    const idx = headers.findIndex(h=>normSelHeader(h)===g);
    if(idx>=0) return idx;
  }
  for(const g of field.guess||[]){
    const idx = headers.findIndex(h=>normSelHeader(h).includes(g));
    if(idx>=0) return idx;
  }
  return -1;
}
...
// ---------- Fabric Requirement — parsing EMBEE's own Fabric Ordering tracker ----------
// Unlike the Decathlon Selection File, this is EMBEE's own internally-maintained workbook,
// so its column headers stay consistent week to week — no flexible mapping screen needed,
// just known header names (with light fuzzy-matching for the couple of long/typo-prone ones).
// One row = one fabric part of one style (a style can have several: Main Fabric, Secondary,
// Main-Top, Secondary-Bottom, etc.), each tied to its own CPT (fabric) and Supplier.
const FABRIC_HEADER_ALIASES = {
  'brand':'brand', 'fabric part':'fabricPart', 'cc':'cc', 'model code':'modelCode', 'model name':'modelName',
  'total commitment':'totalCommitment', 'selection qty':'selectionQtyRef',
  'cpt model code':'cptModelCode', 'cpt name':'cptName', 'cpt item code':'cptItemCode', 'cpt color':'cptColor',
  'fabric sharing':'fabricSharing', 'supplier':'supplier',
  '0% consumption':'baseConsumption', 'units':'units', 'cw':'cw', 'weight':'weight', 'mtsr':'mtsr',
  'stamped hanger status':'hangerStatus', 'fob / cif / ex-w':'incoterm', 'fabric price':'fabricPrice',
  'order date':'orderDate', 'embee internal po#':'internalPo', 'requested ehd':'requestedEhd',
  'total fabric ordered':'totalFabricOrdered',
};
function normFabricHeader(h){ return String(h==null?'':h).trim().toLowerCase(); }
// The two stock columns have long, typo-prone header text in the real file ("Any Fabirc
// STOCK in mtrs/yds/Kgs...", "Any FG Stock in Pcs After Deducting...") — matched by substring
// rather than an exact string so small wording changes next season don't break the import.
function classifyFabricHeader(h){
  const norm = normFabricHeader(h);
  if(FABRIC_HEADER_ALIASES[norm]) return FABRIC_HEADER_ALIASES[norm];
  if(norm.includes('fabirc stock') || norm.includes('fabric stock')) return 'fabricStock';
  if(norm.includes('fg stock')) return 'fgStockPcs';
  if(/^acid/.test(norm)) return 'shipAcid';
  return null;
}
function parseFabricOrderingWorkbook(wb, fileName){
  const ws = wb.Sheets[wb.SheetNames[0]];
  const grid = XLSX.utils.sheet_to_json(ws, {header:1, defval:null, raw:true});
  if(!grid.length) throw new Error('The sheet is empty.');
  const headerRow = grid[0] || [];
  const colMeta = headerRow.map(h=>classifyFabricHeader(h));
  const shipAcidCols = []; // each shipment block is 7 columns starting at an "ACID-n" header
  colMeta.forEach((field,idx)=>{ if(field==='shipAcid') shipAcidCols.push(idx); });
  const rows = [];
  for(let r=1;r<grid.length;r++){
    const row = grid[r];
    if(!row || row.every(c=>c==null||String(c).trim()==='')) continue;
    const rec = {brand:'',fabricPart:'',cc:'',modelCode:'',modelName:'',totalCommitment:0,season:'',
      cptModelCode:'',cptName:'',cptItemCode:'',cptColor:'',fabricSharing:1,supplier:'',
      baseConsumption:0,extraPct:2,units:'',cw:'',weight:'',mtsr:'',hangerStatus:'',incoterm:'',fabricPrice:0,
      orderDate:'',internalPo:'',requestedEhd:'',fabricStock:0,fgStockPcs:0,totalFabricOrdered:0,
      containerCapacity:0, shipments:[]};
    colMeta.forEach((field,idx)=>{
      if(!field || field==='shipAcid') return;
      const cell = row[idx];
      if(['totalCommitment','fabricSharing','baseConsumption','fabricPrice','fabricStock','fgStockPcs','totalFabricOrdered'].includes(field)){
        rec[field] = cell==null? (field==='fabricSharing'?1:0) : Number(cell)||0;
      } else if(field==='orderDate' || field==='requestedEhd'){
        rec[field] = excelDateToISO(cell);
      } else if(field!=='selectionQtyRef'){
        rec[field] = cell==null? '' : String(cell).trim();
      }
    });
    // Each ACID column starts a 7-column shipment block: ACID, Invoice#, Qty, Extra/Shortage, ETD, ETA, In-house date.
    shipAcidCols.forEach(startIdx=>{
      const acid = row[startIdx];
      const qty = row[startIdx+2];
      if((acid==null || String(acid).trim()==='') && (qty==null || Number(qty)===0)) return;
      rec.shipments.push({
        acid: acid==null? '' : String(acid).trim(),
        invoiceNo: row[startIdx+1]==null? '' : String(row[startIdx+1]).trim(),
        qty: qty==null? 0 : Number(qty)||0,
        extraShortage: row[startIdx+3]==null? 0 : Number(row[startIdx+3])||0,
        etd: excelDateToISO(row[startIdx+4]),
        eta: excelDateToISO(row[startIdx+5]),
        inHouseDate: excelDateToISO(row[startIdx+6]),
      });
    });
    if(!rec.modelCode && !rec.cptName) continue; // skip rows with no identifiable style/fabric at all
    rec.id = 'fl_'+Date.now()+'_'+r;
    rows.push(rec);
  }
  if(!rows.length) throw new Error('No fabric line rows found — expected columns like Brand, Fabric Part, Model Code, CPT Name, Supplier, 0% Consumption, Total Commitment.');
  return { id:'frq_'+Date.now(), type:'FABRIC_ORDERING', fileName, uploadedAt:new Date().toISOString(), rowCount:rows.length, rows };
}
// Replicates EMBEE's own Excel formulas exactly, decoded from the real workbook:
//   FG Qty as per Sharing        = Total Commitment × Fabric Sharing
//   With Extra % (Consumption)   = Base Consumption × (1 + Extra % / 100)
//   Total Fabric to be Ordered   = (Total Commitment − FG Stock Pcs) × Consumption(with extra) − Fabric Stock
//   Fabric Ordered for FG Units  = Total Fabric to be Ordered ÷ Consumption(with extra)
//   Total Pending to Order       = Total Fabric Ordered − Total Fabric to be Ordered  (negative = still short)
//   Ttl Shipped Qty              = sum of every shipment's Qty
//   Balance to Ship              = Ttl Shipped Qty − Total Fabric Ordered
// Container estimate is a new addition (not present in the source workbook) — Required Fabric
// ÷ Container Capacity, where Container Capacity is entered per line since EMBEE's own file has
// no such column to read it from.
function computeFabricLineCalc(line){
  const fgQtyAsPerSharing = n(line.totalCommitment) * n(line.fabricSharing);
  const consumptionWithExtra = n(line.baseConsumption) * (1 + n(line.extraPct)/100);
  const totalFabricToBeOrdered = (n(line.totalCommitment) - n(line.fgStockPcs)) * consumptionWithExtra - n(line.fabricStock);
  const fabricOrderedForFgUnits = consumptionWithExtra>0 ? totalFabricToBeOrdered/consumptionWithExtra : 0;
  const totalPendingToOrder = n(line.totalFabricOrdered) - totalFabricToBeOrdered;
  const shippedQtyTotal = (line.shipments||[]).reduce((s,sh)=>s+n(sh.qty),0);
  const balanceToShip = shippedQtyTotal - n(line.totalFabricOrdered);
  const estimatedContainers = n(line.containerCapacity)>0 ? totalFabricToBeOrdered/n(line.containerCapacity) : null;
  return { fgQtyAsPerSharing, consumptionWithExtra, totalFabricToBeOrdered, fabricOrderedForFgUnits,
    totalPendingToOrder, shippedQtyTotal, balanceToShip, estimatedContainers };
}
...
function SelectionColumnMappingScreen({headerRow, previewRows, initialMapping, onConfirm, onBack}){
  ...
  const setField = (key,val)=> setMapping(prev=>({...prev,[key]:val===''? -1 : Number(val)}));
    ...
    <div className="section" style={{marginBottom:16}}>
      ...
      <div className="section-body">
        <div className="foot-note" style={{marginTop:0,marginBottom:14}}>
          ...
          isn't there. This mapping is remembered for this season next time.
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:2}}>
          {SELECTION_MAPPING_FIELDS.map(f=>(
            <div key={f.key} style={{display:'flex',flexWrap:'wrap',alignItems:'center',gap:'6px 14px',padding:'10px 4px',borderBottom:'1px solid var(--border-soft)'}}>
              ...
              <select value={mapping[f.key]==null? -1 : mapping[f.key]} onChange={e=>setField(f.key,e.target.value)}
...
function ExcelGridInput({gridRef}){
  ...
  useEffect(()=>{
    ...
    const instance = window.jspreadsheet(containerRef.current, {
      data: blankExcelGridData(),
      columns: Array.from({length:EXCEL_GRID_COLS}, ()=>({type:'text', width:110})),
      minDimensions: [EXCEL_GRID_COLS, EXCEL_GRID_ROWS],
      tableOverflow: true,
      tableHeight: '420px',
      freezeColumns: 0,
      allowInsertRow: true,
    ...
    gridRef.current = {
      ...
      clear: ()=> instanceRef.current && instanceRef.current.setData(blankExcelGridData()),
    };
    return ()=>{
...
function SelectionFileUploadCard({onImport,parsing,savedMappings,recentSeasons,fixedSeason}){
  ...
  const [mode,setMode] = useState('file'); // 'file' | 'grid'
  ...
  const handlePickFile = async (file)=>{
    setReadError('');
    try{
      const meta = await readSelectionFileHeaders(file);
      setFileMeta(meta);
      setStep('map');
  ...
  const initialMapping = useMemo(()=>{
    ...
    SELECTION_MAPPING_FIELDS.forEach(f=>{
      ...
      m[f.key] = stillValid ? savedIdx : guessSelectionColumn(f, fileMeta.headerRow);
    });
    return m;
    ...
    <div className="section" style={{marginBottom:16}}>
      ...
      <div className="section-body">
        ...
        <div style={{display:'flex',gap:6,marginBottom:12}}>
          <button type="button" className={"btn"+(mode==='file'?" primary":"")} onClick={()=>setMode('file')}>Upload file</button>
          <button type="button" className={"btn"+(mode==='grid'?" primary":"")} onClick={()=>setMode('grid')}>Excel-style grid</button>
        </div>

        {mode==='file' ? (
          <div className="drop" style={!seasonReady?{opacity:.5,pointerEvents:'none'}:undefined} onClick={()=>seasonReady && inputRef.current.click()}>
            <input ref={inputRef} type="file" accept=".xlsx,.xls" disabled={!seasonReady} onChange={e=>{
              const f = e.target.files[0]; if(f) handlePickFile(f); e.target.value='';
            }} />
            <div className="drop-title">{parsing? 'Parsing…' : seasonReady? `Upload Selection File for ${season}` : 'Enter a season above first'}</div>
            <div>Every season's file can have different column names — after choosing a file, you'll confirm which column maps to which field before anything is imported.</div>
          </div>
        ) : (
...
function SeasonWorkspaceTabs({season, currentUser,
  selectionBatches, onImport, parsingSelection, onDeleteSelection, savedMappings, seasonNames,
  fabricLines, fabricLineBatches, onFabricUpload, parsingFabric, onDeleteFabricBatch, onUpdateFabricLine, onDeleteFabricLine, onAddFabricLine,
  ...
  const seasonFabricBatches = useMemo(()=>fabricLineBatches.filter(b=>b.season===season),[fabricLineBatches,season]);
    ...
    <div>
      ...
      {tab==='selection' && (
        <SelectionFilePage key={season} selectionBatches={selectionBatches} onImport={onImport} parsingSelection={parsingSelection}
          onDeleteSelection={onDeleteSelection} savedMappings={savedMappings} seasonNames={seasonNames} fixedSeason={season} />
      )}
      {tab==='yarn' && <YarnRequirementPage key={season} />}
      {tab==='fabric' && (
        <FabricRequirementPage key={season} fabricLines={seasonFabricLines} fabricLineBatches={seasonFabricBatches}
          onUpload={onFabricUpload} parsing={parsingFabric} onDeleteBatch={onDeleteFabricBatch}
          onUpdateLine={onUpdateFabricLine} onDeleteLine={onDeleteFabricLine} onAddLine={onAddFabricLine} season={season} />
      )}
...
function SeasonNamesPage({seasonNames,onAdd,onRemove, currentUser,
  selectionBatches, onImport, parsingSelection, onDeleteSelection, savedMappings,
  fabricLines, fabricLineBatches, onFabricUpload, parsingFabric, onDeleteFabricBatch, onUpdateFabricLine, onDeleteFabricLine, onAddFabricLine,
  accessoryStyles, setAccessoryStyles, firmOrderBatches, shipmentBatches,
}){
    ...
    return (
      <div>
        ...
        <SeasonWorkspaceTabs season={activeSeason} currentUser={currentUser}
          selectionBatches={selectionBatches} onImport={onImport} parsingSelection={parsingSelection} onDeleteSelection={onDeleteSelection} savedMappings={savedMappings} seasonNames={seasonNames}
          fabricLines={fabricLines} fabricLineBatches={fabricLineBatches} onFabricUpload={onFabricUpload} parsingFabric={parsingFabric} onDeleteFabricBatch={onDeleteFabricBatch} onUpdateFabricLine={onUpdateFabricLine} onDeleteFabricLine={onDeleteFabricLine} onAddFabricLine={onAddFabricLine}
...
// Shipment sub-editor — a flexible list rather than a fixed 7-block layout, since real usage
// varies from zero to several partial shipments per fabric line.
function FabricShipmentEditor({shipments, onChange}){
  const setField = (idx,field,val)=> onChange(shipments.map((s,i)=> i===idx? {...s,[field]:val} : s));
  const addRow = ()=> onChange([...shipments, {acid:'',invoiceNo:'',qty:0,extraShortage:0,etd:'',eta:'',inHouseDate:''}]);
  const removeRow = (idx)=> onChange(shipments.filter((_,i)=>i!==idx));
  return (
    <div className="table-scroll">
      <table className="po-mini-table">
        <thead><tr><th className="lbl">ACID</th><th className="lbl">Invoice #</th><th>Qty</th><th>Extra/Shortage</th><th className="lbl">ETD</th><th className="lbl">ETA</th><th className="lbl">In-house Date</th><th></th></tr></thead>
        <tbody>
          {shipments.map((s,idx)=>(
            <tr key={idx}>
              <td><input type="text" value={s.acid} onChange={e=>setField(idx,'acid',e.target.value)} /></td>
              <td><input type="text" value={s.invoiceNo} onChange={e=>setField(idx,'invoiceNo',e.target.value)} /></td>
              <td><input type="number" value={s.qty} onFocus={e=>e.target.select()} onChange={e=>setField(idx,'qty',Number(e.target.value)||0)} /></td>
              <td><input type="number" value={s.extraShortage} onFocus={e=>e.target.select()} onChange={e=>setField(idx,'extraShortage',Number(e.target.value)||0)} /></td>
              <td><input type="date" value={s.etd} onChange={e=>setField(idx,'etd',e.target.value)} /></td>
              <td><input type="date" value={s.eta} onChange={e=>setField(idx,'eta',e.target.value)} /></td>
              <td><input type="date" value={s.inHouseDate} onChange={e=>setField(idx,'inHouseDate',e.target.value)} /></td>
              <td><button className="icon-btn" onClick={()=>removeRow(idx)}>✕</button></td>
            </tr>
          ))}
          {!shipments.length && <tr><td colSpan={8} className="empty" style={{padding:10}}>No shipments logged yet.</td></tr>}
        </tbody>
      </table>
      <button type="button" className="add-col-btn" style={{marginTop:6}} onClick={addRow}>+ Add shipment</button>
    </div>
  );
}
  ...
  return (
    <React.Fragment>
      <tr>
        ...
        <td><button className="icon-btn" title="Remove line" onClick={()=>{ if(window.confirm('Remove this fabric line? This can\'t be undone.')) onDelete(line.id); }}>✕</button></td>
      </tr>
      {expanded && (
        <tr>
...
function FabricRequirementPage({fabricLines,fabricLineBatches,onUpload,parsing,onDeleteBatch,onUpdateLine,onDeleteLine,onAddLine,season}){
  const [search,setSearch] = useState('');
  ...
  const suppliers = useMemo(()=>[...new Set(fabricLines.map(l=>l.supplier).filter(Boolean))].sort(),[fabricLines]);
  const brands = useMemo(()=>[...new Set(fabricLines.map(l=>l.brand).filter(Boolean))].sort(),[fabricLines]);
  const parts = useMemo(()=>[...new Set(fabricLines.map(l=>l.fabricPart).filter(Boolean))].sort(),[fabricLines]);

  const filtered = useMemo(()=>fabricLines.filter(l=>
    (!supplierFilter || l.supplier===supplierFilter) &&
    (!brandFilter || l.brand===brandFilter) &&
    (!partFilter || l.fabricPart===partFilter) &&
    (!search || (l.modelCode+' '+l.modelName+' '+l.cptName).toLowerCase().includes(search.toLowerCase()))
  ),[fabricLines,supplierFilter,brandFilter,partFilter,search]);

  const totals = useMemo(()=>{
    return filtered.reduce((a,l)=>{
      const c = computeFabricLineCalc(l);
      a.toBeOrdered += c.totalFabricToBeOrdered; a.ordered += n(l.totalFabricOrdered);
      a.pending += c.totalPendingToOrder; a.containers += (c.estimatedContainers||0);
      return a;
    },{toBeOrdered:0,ordered:0,pending:0,containers:0});
  },[filtered]);
  ...
  const summaryRows = useMemo(()=>{
    ...
    const map = {};
    filtered.forEach(l=>{
      const key = l[summaryBy] || 'Unspecified';
      const c = computeFabricLineCalc(l);
      if(!map[key]) map[key] = {key, lines:0, toBeOrdered:0, ordered:0, pending:0, containers:0};
      map[key].lines++; map[key].toBeOrdered += c.totalFabricToBeOrdered; map[key].ordered += n(l.totalFabricOrdered);
      map[key].pending += c.totalPendingToOrder; map[key].containers += (c.estimatedContainers||0);
    });
  ...
  return (
    <div>
      <div className="section" style={{marginBottom:16,background:'var(--primary-soft)',border:'1px solid var(--primary)'}}>
        <div className="section-body" style={{fontSize:13,color:'var(--ink-soft)'}}>
          Upload EMBEE's own Fabric Ordering workbook each week — consumption, requirement, stock netting,
          ordered/pending and shipment tracking all calculate automatically using the same formulas as the source file.
        </div>
      </div>

      <div className="section" style={{marginBottom:16}}>
        <div className="section-head"><div className="section-title">Upload Fabric Ordering File</div></div>
        <div className="section-body">
          <TaggedUploadDrop label={parsing? 'Parsing…' : 'Upload Fabric Ordering Excel'}
            hint="Reads Brand, Fabric Part, Model Code, CPT Name/Color, Supplier, 0% Consumption, Total Commitment, stock, ordered qty and shipment columns automatically."
            onFile={(f,weekLabel)=>onUpload(f,weekLabel,season)} parsing={parsing} batches={fabricLineBatches} />
          {fabricLineBatches.length>0 && (
            <div className="table-scroll" style={{marginTop:12}}>
              <table>
                <thead><tr><th>Week</th><th>File</th><th className="num">Rows</th><th>Uploaded</th><th>By</th><th></th></tr></thead>
                <tbody>
                  {fabricLineBatches.slice().reverse().map(b=>(
                    <tr key={b.id}>
                      <td><span className="badge-count">{b.weekLabel}</span></td>
                      <td>{b.fileName}</td>
                      <td className="num">{fmt(b.rowCount)}</td>
                      <td>{new Date(b.uploadedAt).toLocaleString()}</td>
                      <td>{b.uploadedBy||'—'}</td>
                      <td><button className="icon-btn" title="Remove upload record (lines already imported stay in the table below)"
                        onClick={()=>{ if(window.confirm('Remove this upload record? The fabric lines it added stay in the table below — remove those individually if needed.')) onDeleteBatch(b.id); }}>✕</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="kpi-grid">
        ...
        <KpiCard label="Total Fabric Ordered" value={fmt(totals.ordered.toFixed(0))} />
        <KpiCard label="Total Pending to Order" value={fmt(totals.pending.toFixed(0))} tone={totals.pending<0?'red':'green'} foot="Negative = still short" />
        <KpiCard label="Estimated Containers" value={totals.containers.toFixed(1)} />
      </div>

      <div className="section">
        <div className="section-head">
          <div className="section-title">Fabric Lines <span className="badge-count">{filtered.length}</span></div>
          <button className="btn primary" onClick={()=>onAddLine(season)}><Icon name="plus" size={14}/> Add line</button>
        </div>
        <div className="section-body">
          <div className="filter-row">
          ...
          {summaryBy ? (
            <div className="table-scroll">
              <table>
                <thead><tr><th>{summaryBy==='modelCode'?'Style / Model':summaryBy==='supplier'?'Supplier':'Brand'}</th><th className="num">Lines</th><th className="num">To be Ordered</th><th className="num">Ordered</th><th className="num">Pending</th><th className="num">Est. Containers</th></tr></thead>
                <tbody>
                  {summaryRows.map(r=>(
                    <tr key={r.key}>
                      ...
                      <td className="num">{fmt(r.ordered.toFixed(0))}</td>
                      <td className="num" style={{color:balColor(r.pending)}}>{fmt(r.pending.toFixed(0))}</td>
                      <td className="num">{r.containers.toFixed(1)}</td>
                    </tr>
                  ))}
                  {!summaryRows.length && <tr><td colSpan={6} className="empty">No fabric lines match this filter.</td></tr>}
                </tbody>
              </table>
            </div>
          ) : !filtered.length ? <div className="empty">No fabric lines yet — upload the Fabric Ordering file above, or add a line manually.</div> : (
            <div className="table-scroll">
              <table className="grid-table">
                <thead>
                  <tr>
                    <th className="stickycol lbl">Style / Model</th><th className="lbl">Fabric Part</th><th className="lbl">Fabric / Color</th><th className="lbl">Supplier</th>
                    <th>Total Commitment</th><th>Fabric Sharing</th><th>FG Qty</th>
                    <th>Base Consumption</th><th>Extra %</th><th>Consumption+Extra</th>
                    <th>Fabric Stock</th><th>FG Stock (Pcs)</th><th>To be Ordered</th><th>Fabric Ordered</th><th>Pending</th>
                    <th>Container Cap.</th><th>Est. Containers</th><th>Shipped</th><th>Balance to Ship</th><th></th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(l=>(
                    <FabricLineRow key={l.id} line={l} expanded={expandedId===l.id}
          ...
          <div className="foot-note">
            Formulas match EMBEE's own Fabric Ordering workbook: FG Qty = Total Commitment × Fabric Sharing · Consumption+Extra = Base × (1 + Extra% / 100) ·
            To be Ordered = (Total Commitment − FG Stock) × Consumption+Extra − Fabric Stock · Pending = Fabric Ordered − To be Ordered (negative = still short) ·
            Est. Containers = To be Ordered ÷ Container Capacity (enter Container Capacity per line — not present in the source file).
          </div>
        </div>
      </div>
    </div>
...
export default function App(){
  ...
  const [fabricLineBatches,setFabricLineBatches] = useState([]);
  const [parsingFabricOrdering,setParsingFabricOrdering] = useState(false);
  ...
  useEffect(()=>{
    (async ()=>{
      ...
      const fBatches = await storeGet('fabric-line-batches',true);
      ...
      if(fBatches) setFabricLineBatches(fBatches);
  ...
  const handleFabricOrderingUpload = useCallback(async (file, weekLabel, season)=>{
    setParsingFabricOrdering(true);
    try{
      const buf = await file.arrayBuffer();
      const wb = XLSX.read(buf,{type:'array', cellDates:true});
      const batch = parseFabricOrderingWorkbook(wb, file.name);
      batch.rows.forEach(row=>{ row.season = season||''; });
      setFabricLines(prev=>{
        const next = [...prev, ...batch.rows];
        storeSet('fabric-lines', next, true);
        return next;
      });
      setFabricLineBatches(prev=>{
        const next = [...prev, {id:batch.id, fileName:batch.fileName, uploadedAt:batch.uploadedAt, rowCount:batch.rowCount,
          weekLabel: weekLabel||'Unspecified', season: season||'', uploadedBy: currentUser? currentUser.displayName : ''}];
        storeSet('fabric-line-batches', next, true);
        return next;
      });
    }catch(e){ alert('Could not parse this file: '+e.message); }
    setParsingFabricOrdering(false);
  },[currentUser]);
  ...
  const handleAddFabricLine = useCallback((season)=>{
    setFabricLines(prev=>{
      const blank = {id:'fl_'+Date.now(), brand:'',fabricPart:'',cc:'',modelCode:'',modelName:'',totalCommitment:0,season:season||'',
        cptModelCode:'',cptName:'',cptItemCode:'',cptColor:'',fabricSharing:1,supplier:'',
        baseConsumption:0,extraPct:2,units:'',cw:'',weight:'',mtsr:'',hangerStatus:'',incoterm:'',fabricPrice:0,
        orderDate:'',internalPo:'',requestedEhd:'',fabricStock:0,fgStockPcs:0,totalFabricOrdered:0,
        containerCapacity:0, shipments:[]};
  ...
  const handleDeleteFabricLineBatch = useCallback((batchId)=>{
    setFabricLineBatches(prev=>{
      ...
      <div className="main">
        ...
        <div className="content">
          ...
          {page==='seasonnames' && <SeasonNamesPage seasonNames={seasonNames} onAdd={handleAddSeasonName} onRemove={handleRemoveSeasonName} currentUser={currentUser}
            selectionBatches={selectionBatches} onImport={handleSelectionUpload} parsingSelection={parsingSelection} onDeleteSelection={handleDeleteSelectionBatch} savedMappings={selectionMappings}
            fabricLines={fabricLines} fabricLineBatches={fabricLineBatches} onFabricUpload={handleFabricOrderingUpload} parsingFabric={parsingFabricOrdering} onDeleteFabricBatch={handleDeleteFabricLineBatch}
            onUpdateFabricLine={handleUpdateFabricLine} onDeleteFabricLine={handleDeleteFabricLine} onAddFabricLine={handleAddFabricLine}
            accessoryStyles={accessoryStyles} setAccessoryStyles={setAccessoryStyles} firmOrderBatches={firmOrderBatches} shipmentBatches={shipmentBatches}
          />}
